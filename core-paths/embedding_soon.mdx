...
To get started with the embedded experience, it is important to consider two concepts:

1. The space component which has [Workbooks](../concepts/workbooks)
2. [Events](../concepts/events) which enables workbooks to have asynchronous customizable jobs to happen either behind the scenes or with user interaction.
