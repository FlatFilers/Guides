...
This is the section where you specify the responses to events. It is also the place where you can handle the transformation and validation of your data. If you're interested in using our recordHook plugin, you can obtain it by downloading it from npm @flatfile/plugin-record-hook.
Record-level hooks provide access to all the fields in a row and should be used for operations that require access to multiple fields or when creating a new field. [Example-below]
