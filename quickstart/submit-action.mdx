---
title: Configure a submit Action
description: "Learn how to create a simple submit Action in Flatfile."
icon: "circle-3"
---

Actions refer to buttons added to the user interface that trigger <Tooltip tip="Large asynchronous work...">[Jobs](../concepts/jobs)</Tooltip> within the Flatfile platform when clicked by users. They enable you to run complex operations and streamline workflows. They are declared as part of the Blueprint that defines a Workbook.

<Tip>
  Use your [secret
  key](../developer-tools/security/authentication#secret-and-publishable-keys)
  from the `development` environment. Secret key format is "sk_abcd..." and is
  found in [your Flatfile Dashboard
  here](https://platform.flatfile.com/developers).
</Tip>

In this example, we're going to add behavior to our Workbook's primary Action (`submitAction`).

### Adding behavior to the action

When the button is clicked by a user, a job is created. We will [listen for that job to be ready](../concepts/events#job-ready), and our code will call out to an external webhook receiver provided by the [webhook.site](https://webhook.site) service.

1. To create the receiver, navigate to [webhook.site](https://webhook.site)
1. Copy the value of the path after webhook.site for `Your unique URL`.
1. Next, add the following to your default export function.

<Snippet file="shared/workbook_submit_pt2.mdx" />

#### Testing your Action

1. With the `develop` command still running in terminal, head to your Workbook and click the submit Action in the top right corner.
2. In webhook.site, look at the query log and observe that the webhook received a call; this is the Action running.

---

## Recap & Next Steps

Now that we are listening for an action button to be clicked, let's hop into the UI and review your new Workbook and add data to it. Then, we can test submitting again with data!
