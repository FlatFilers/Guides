...
Every one of these actions creates an [Event](../concepts/events). You can author code to listen for these events and add handlers that perform tasks such as validating records or sending data to your own API.
...
1. After starting the code in repl.it, return to your Contacts sheet and modify an Email Address value to make it an invalid email. 
2. You should notice that when you take focus off of the field, your validation will run and the sheet user interface will display an error for the changed field.
