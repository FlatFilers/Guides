...
Every one of these actions creates an [Event](../concepts/events). You can author code to listen and react to these events by performing tasks such as validating records or sending data to your own API.

## Setting up a listener

<Note>Flatfile provides a powerful Typescript/NodeJS SDK for working with events and many plugins. There are a lot of ways to listen to events in other ways that we describe in the [Events](../concepts/events) docs here.</Note>
...
Listeners are the basic building block of logic in Flatfile. They span from simple validations and transformations to complex, multi-sheet workflows.
...
In this example, we'll use the [@flatfile/plugin-record-hook](https://www.npmjs.com/package/@flatfile/plugin-record-hook) plugin to notify us on every row addition or modification and give us the ability to modify the data. For this example, we will take the first name record and make it lowercase.
...
1. After starting the code in repl.it, go back to your Contacts worksheet.
2. Manually add a new row and write a name in the First Name field written in all capital letters. 
3. Take focus off the field you entered (click outside that field). 

Once you take focus off of the new First Name field, your transformation will run and convert the characters in your value to lower case.
