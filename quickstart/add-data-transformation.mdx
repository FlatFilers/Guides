...
Next, we will <Tooltip tip="Configure a submit action">[add an action](submit-action)</Tooltip> that will call out to an external webhook receiver provided by the <Tooltip tip="Test webhook">[webhook.site](https://webhook.site)</Tooltip> service.
