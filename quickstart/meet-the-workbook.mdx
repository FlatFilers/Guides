---
title: 1. Meet the workbook
description: "Learn how to create your first Workbook in Flatfile."
---
A workbook in Flatfile replaces the spreadsheet template you may share 
with people today when requesting data during the data collection phase 
of customer onboarding or other business processes. Unlike a spreadsheet, 
it's designed to allow your team and users to validate, correct, and import data.


In the next 15 minutes, you’ll design and configure a workbook 
for your business. Once you’re finished you’ll be able to embed it in your application 
as an import flow, or deploy a data collection portal.



<a target="_blank" class="group px-4 py-1.5 relative inline-flex items-center rounded-full shadow-sm text-sm font-medium" 
href="https://github.com/FlatFilers">
    <span class="absolute inset-0 bg-primary-dark rounded-full group-hover:opacity-[0.9]"></span>
    <span class="z-10 mr-2 text-white">Download full app</span>
    <svg width="6" height="3" class="h-2 overflow-visible -rotate-90 text-white" aria-hidden="true"><path d="M0 0L3 3L6 0" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"></path></svg>
</a>

<a target="_blank" href="https://replit.com/@AshleyMulligan/BogusFinancialAssembler#index.ts" class="font-medium hover:text-primary dark:hover:text-primary-light py-1.5 mx-4">See it in Replit</a>


#### Using Workbooks

The Workbook brings together automation and humans to get data into its final shape. 

<img src="../images/quickstart/workbook.png" caption="The Flatfile Workbook"/>

With Workbooks, you can:
1. Accept data from any of over N supported file types. (And you can write your own file extractors if that's not enough.)
2. Automatically apply any validation rules a developer has previously defined.
3. Provide end users the ability to add, remove, review, filter, and correct any data imported into a Workbook.
4. Define [in code] at least one primary action that submits the reviewed data to a destination API, database, or workflow step of your choosing.


## 1.1 Building your first Workbook

Flatfile supports using many different schema DDLs. If you have an existing schema, use plugins like
  [@flatfile/plugin-json-schema](hello.com), [@flatfile/plugin-sql-ddl](hello.com), [@flatfile/plugin-openapi](hello.com), [@flatfile/plugin-graphql](hello.com).
  
  For this tutorial, we're going to create a simple schema using <Tooltip tip="Learn more about Blueprint">**Blueprint**</Tooltip>. Quickly create a sample Workbook by running this curl API request in your terminal or you can all our API directly from our API reference Docs.

<a target="_blank" class="group px-4 py-1.5 relative inline-flex items-center rounded-full shadow-sm text-sm font-medium" 
href="https://flatfile.stoplight.io/docs/v10/branches/ashley-mulligan-sample-workbook/tjt005wdogklo-create-a-sample-workbook">
    <span class="absolute inset-0 bg-primary-dark rounded-full group-hover:opacity-[0.9]"></span>
    <span class="z-10 mr-2 text-white">Run directly from our Docs</span>
    <svg width="6" height="3" class="h-2 overflow-visible -rotate-90 text-white" aria-hidden="true"><path d="M0 0L3 3L6 0" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"></path></svg>
</a>

```shell
curl --request POST \
  --url https://api.x.flatfile.com/v1/workbooks \
  --header 'Authorization: Bearer ${publishableKey}' \
  --header 'Content-Type: application/json' \
  --data '{
  "name": "My First Workbook",
  "blueprint": {
    "sheets": [
      {
        "name": "My Sheet 1",
        "description": "hello",
        "slug": "mysheet1",
        "fields": [
          {
            "key": "first_name",
            "type": "string",
            "label": "First Name"
          },
          {
            "key": "last_name",
            "type": "string",
            "label": "Last Name"
          },
          {
            "key": "full_name",
            "type": "string",
            "label": "Full Name"
          }
        ]
      }
    ]
  }
}'
```

### Recap & Next Steps
You should now have a Workbook in your Account. Head over to your [Dashboard](https://dashboard.flatfile.com) to see it and interact with it. 

Next, we'll bring your Workbook to life by adding data transformations and validations to it. 

