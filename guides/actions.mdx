...
Listen for a `job:ready` and filter on the `domain` (workbook) and the `operation` of where the action was placed. Be sure to complete the job when it's complete.
