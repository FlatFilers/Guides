...
<Info>
  If you configure `constraints: [{ type: 'hasSelection' }]` on an Action, it will disable a
  Sheet Action when no records in the Sheet are selected.

If you configure `constraints: [{ type: 'hasAllValid' }]` on an Action, it will disable a
Sheet Action when there are invalid records.
