...
<Info>
  If you configure `constraints: [{ type: 'hasSelection' }]` on an Action, it
  will disable a Sheet Action when no records in the Sheet are selected.

If you configure `constraints: [{ type: 'hasAllValid' }]` on an Action, it will
...
Get the example in the [getting-started](https://github.com/FlatFilers/flatfile-docs-kitchen-sink/blob/main/typescript/dynamic-configurations/index.ts) repo
