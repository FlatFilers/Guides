...
First, configure your action to have inputs on your Blueprint. These will appear once the action button is clicked.

<Snippet file="shared/workbook_submit_pt4.mdx" />

Next, listen for a `job:ready` and filter on the `job` you'd like to process. Grab the data from the job itself, then be sure to complete the job when it's complete.
