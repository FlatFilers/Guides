...
When the **Create New Space** button is clicked in the platform, this spawns a [job event](../concepts/events#job-ready) with a `configure` operation in the `space` domain. This is the equivalent of making a POST operation to `/api/v1/spaces` with `autoconfigure: true`.
...
```jsx listener.js
listener.filter({ job: "space:configure" }, ({ on }) => {
  on("job:ready", async ({ context: { spaceId, environmentId, jobId } }) => {
    try {
