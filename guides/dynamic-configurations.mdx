...
When the "Create New Space" button is clicked on the platform, it triggers a [job `event`](../concepts/events) within the **space** `domain` that initiates a **configure** `operation`. This operation is analogous to performing a POST request to the endpoint [/api/v1/spaces](https://flatfile.stoplight.io/docs/api/25e20c8ab61c5-create-a-space) with the parameter `autoconfigure` set to true.
