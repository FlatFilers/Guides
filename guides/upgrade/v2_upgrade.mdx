...
<Note>
    When providing you select/enum options there is still an `options` array that is shaped the same, however, one thing to note is that this options array has been moved into a new field that is `config` for Platform.
</Note>
...
In Portal, you were able to specify a webhook endpoint using `webhookUrl` -OR- use the returned data within the Promise to do whatever needed to be done with your data. With Platform, we use the Listners to wait for the submit action to happen and then act on it.
