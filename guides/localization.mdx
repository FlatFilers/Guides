...
<Info>
Flatfile will auto-detect which language to use from the users browser settings! If the users language is undetectable or we dont currently support it, we will fallback to english.
</Info>
...
yourTranslationsDirectory/
    └── locales/
        ...
        ...
```
<Info>We automatically determine which language to display based on the users browser settings.</Info>
