...
Flatfile will expire Spaces (the Space will no longer be accessible but will still appear in the Spaces' response) and delete any data that was uploaded or added to the respective Space.
...
- If Flatfile's cron event runs on January 2nd, any Space created before January 1st (older than 2 days) will be expired. Any data that was uploaded to the Spaces will be deleted.

### Last Activity Policy Example:

- Space last modified on January 1st.
- Policy set to 3 days.
- If Flatfile's cron event runs on January 4th, any Space that hasn't been modified since January 1st (older than 3 days) will be expired. Any data that was uploaded to the Spaces will be deleted.
