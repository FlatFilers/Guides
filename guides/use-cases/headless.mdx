...
<Note>
  To enable the `commit:completed` event, add a settings parameter in your Sheet that includes trackChanges: true.
  This has the effect of disabling actions on both Sheets and Workbooks until any pending commits have been completed.
