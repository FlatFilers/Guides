...
The primary path for configuring Flatfile is by setting up a server side listener that listens for a Space to be opened or created; however, you can also create a listener that will live in your browser. This can live alongside a server-side listener or on its own. These properties exist for client-side server setup.
