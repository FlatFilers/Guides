...
The importer takes the following options:

### URL Overrides

URL Overrides enabls you to connect with the Flatfile API from a different location, whether for specialized security measures or integration purposes.
...
While setting up embedded Flatfile, you have the option to either establish a fresh Space during each initialization or reuse an existing Space. These attributes specifically apply when you're in the process of crafting a new Space.
...
During the configuration of embedded Flatfile, you have the flexibility to either reuse an existing Space or generate a new one. These aspects come into play when you're working with an already established Space.
...
<ParamField path="iframeStyles" type="CSSProperties">
  Style the iframe using CSS Properties [Learn More](#iframeStyles)
</ParamField>
...
<Warning>Currently only available in the `@flatfile/react` wrapper.</Warning>
