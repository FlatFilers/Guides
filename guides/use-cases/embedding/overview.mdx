...
All embedded Flatfile importers take the following shared properties.
...
<ParamField path="space" type="{id: string, accessToken: string}" required>
  Pass in an existing Space instead of creating a new one each time. This looks
  for your `spaceId` and your `secretKey`.
</ParamField>
...
The CSS here is for styling how you want things to look outside of the Flatfile iframe, like the exit dialog and the dialog wrapper. This css can go in your `style.css` in your `public` folder. Remember to use `!important` to override values.
