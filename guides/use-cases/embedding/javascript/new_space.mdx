...
<button
  class="contrast"
  onclick="initializeNewFlatfileSpace({ publishableKey: 'pk_1234', environmentId: 'us_env_1234' })"
>
  Create new Space
</button>
```
...
Start your front end by heading to terminal and running the following command. To see that it’s running, visit: https://localhost:1234 (or the port it is running on) and you should see your page and a button.
...
In your client.js, at minimum you'll need a `publishableKey` and an `environmentId`. You can also pass it any additional [options](../reference).
