...
Finally, let's get the data out.

1. We'll add a new `listener.filter({ job: "workbook:submitActionFg" })` to listen for a button click and egress data to your destination.
2. Add a `closeSpace` prop to tell the importer when it's complete (when the submit button is clicked).
