...
/* Settings for primary button (confirmation pop up) */
.flatfile_button.flatfile_primary {
    background-color: #4c48ef;
}

/* Settings for secondary button (confirmation pop up) */
.flatfile_button.flatfile_secondary {
    background-color: whitesmoke;
}

/* X close button */
.flatfile-close-button {
    /* set this to "block" to see the X close button. To completely hide it, use "none" */
    display: block;
    /* distance between edge of iFrame and the X close button*/
    margin: 25px !important;
}

.flatfile-close-button svg {
    /* color of the X button to close the iFrame */
