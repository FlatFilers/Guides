...
In your `App.tsx`, at minimum, you'll need to pass in the `publishableKey` and the `environmentId`. Also, add the content here to your `index.tsx`, `tsconfig.json`, `manifest.json`, and `config-overries.js`.
...
2. Now, start your front end by heading to terminal and running the following command.
...
3. To see that it’s running, visit: https://localhost:3000 (or the port it is running on) and you should see your page and a button. Click the button and see that an empty Space gets created.
...
You can control the exact moment when you want to create/use a space. For this, you can use useSpaceTrigger hook, as shown in the example below:
