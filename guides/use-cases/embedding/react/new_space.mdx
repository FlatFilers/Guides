...
Start your front end by heading to terminal and `npm start`. To see that it’s running, visit: https://localhost:1234 and you should see your page and a button.
...
When the “Open Flatfile” button is clicked on your platform, it triggers a job event within the Space domain that initiates a configure operation. This operation (`space:configure`) is analogous to performing a `POST` request to the endpoint `/api/v1/spaces` with the parameter `autoconfigure` set to `true`.

Follow [this guide](../../../../guides/dynamic-configurations) to kickstart an event listener where you'll listen for that job and configure your Space on the fly.
