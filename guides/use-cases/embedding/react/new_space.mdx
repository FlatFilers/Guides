...
In your `App.tsx`, at minimum, you'll need to pass in the `publishableKey` and the `environmentId`. Also, add the content here to your `index.tsx`, `tsconfig.json` and `manifest.json`.
...
1. You'll get an alert: We're unable to detect target browsers. Would you like to add the defaults to your package.json? Say yes.

2. To see that it’s running, visit: https://localhost:3000 (or the port it is running on) and you should see your page and a button. Click the button and see that an empty Space gets created.
