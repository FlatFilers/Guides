...
In your environment file, you'll need to define your keys. In this example, you will need your `environmentId` and `publishableKey`. You can find these under "Developer settings" when logged into [platform.flatfile.com](https://platform.flatfile.com).
...
In your `App.vue` you'll need to pass in a minimum of the `publishableKey` and `environmentId` from your `.env`.
...
Once you add this code, when a change occurs, we'll log the entered first name and update the last name to "Rock." You'll immediately see this begin to work when you add or update any records. Learn more about [Handling Data](/docs/guides/handling-data)
...
By attaching a `themeConfig` to `spaceProps` in `src/App.tsx`, we will now override colors in your Space to match your brand. See all of the options here in the [Theming Reference](/docs/guides/theming).
