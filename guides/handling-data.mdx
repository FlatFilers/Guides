...
When a record is created or updated, you may need to send data to an API. Sends the payload data from an event to a webhook receiver URL using an HTTP POST request, then update the record object based on the success or failure of the request. Last, adds messages to the  fields accordingly.
