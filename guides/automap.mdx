...
Upon uploading a file, after file extraction is complete, the data needs to be mapped to a destination Workbook. In non-headless environment a human might do this manually. But in a headless environment we'll automate this process.
...
<br></br>
<Note>
  Note you may also set `debug` to true for useful error messages in devlopment.
</Note>
