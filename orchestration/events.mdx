...
<ParamField path="snapshot:created" type="readonly">
  Called when a snapshot is created of the sheet. This runs in conjunction with
  AI Assist or the API endpoint is used.
</ParamField>
...
<ParamField path="commit:completed" type="readonly">
  Commit:completed only runs when trackChanges is enabled on the workbook. This
  fires when a commit has completed and any record changes have finished.
</ParamField>
