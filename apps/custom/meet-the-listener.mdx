...
Each Secret Key is scoped to an environment, so you will also need to update the `FLATFILE_ENVIRONMENT_ID` value to the one found on the same page as your Secret Key.
