...
When creating or updating Actions, the following fields are required:

- `targetId`: The ID of the target resource (e.g., workbook, sheet, or file)
- `Id`: Unique identifier for the action
- `createdAt`: Timestamp when the action was created
- `updatedAt`: Timestamp when the action was last updated

These fields are automatically managed by the system but should be included in your action configurations.
