...
A key component of Flatfile's headless automation workflow is the automapping functionality provided by our [Automap Plugin](https://flatfile.com/plugins/plugin-automap/).
...
<Note>
  See our [Extractor Plugins](https://flatfile.com/plugins/category/extractors/) for plug and play
...
To prevent that redirect, set `settings: {noMappingRedirect: true}` in your [Workbook blueprint](../apps/workbooks#settings-object).
</Note>
...
<Note>
  To enable the `commit:completed` event, add a settings parameter in your Workbook
