...
<Info>
  ...
  guide](/apps/custom/meet-the-workbook#building-your-first-workbook).
</Info>
...
```jsx 
const email = record.get("email"); const validEmailAddress = /^[^\s@]+@[^\s@]+.[^\s@]+$/;
if (email !== null && !validEmailAddress.test(email)) {
...
For clearing an entire sheet of its records, set up a bulk delete job. This task will comprehensively wipe out every record on the specified sheet. Check out our [jobs documentation](/orchestration/jobs).
