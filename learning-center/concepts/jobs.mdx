...
<Tooltip tip="A collection of fields...">
  [Sheet](../blueprint/overview#terms)
</Tooltip>
...
activity. Jobs are then managed via [Listeners](/orchestration/listeners), which
receive [Events](/orchestration/events) published by Flatfile in response to
activity.
...
First, create an [Action](/orchestration/actions) on a domain. Here's an example
...
[Listener](/orchestration/listeners), but the most common event to listen for is
