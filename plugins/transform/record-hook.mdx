...
This plugin offers a convenient way to execute custom logic on individual data records within Flatfile. By setting up an event listener for the commit:created event, this plugin seamlessly integrates with the data processing flow.
...
Pass `bulkRecordHook` to a Flatfile data listener and provide a function to run when data is added or updated.
...
The bulkRecordHook can accept additional properties. Props will be passed along to the transformer.
...
Additionally, it passes a `chunkSize` of 100 and asks the hooks to run 2 at a time via the `parallel` property.
