...
<ParamField path="sheetFilter" type="object" required>
  Use the `sheetFilter` parameter to select specific sheets for the plugin to
  monitor. You must provide either a `sheetId` or a `sheetSlug`. The `sheetId`
  specifies the unique ID of the sheet you want to monitor. The `sheetSlug`
  indicates the unique slug name of the sheet you want to monitor.
</ParamField>
