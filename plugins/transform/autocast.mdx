---
title: "@flatfile/plugin-autocast"
description: "A plugin for automatically casting values in Flatfile."
icon: "download"
---

<Warning>
  When embedding Flatfile, this plugin should be deployed in a server-side
  listener. [Learn
  more](../../developer-tools/client_server_side#server-side-listeners)
</Warning>

<CardGroup cols={1}>
  <Card title="@flatfile/plugin-autocast" icon="download">
    <br />
    The `@flatfile/plugin-autocast` plugin is an opinionated transformer that will
    automatically convert the data in the Sheet to match the type defined by the
    Blueprint.
    <br />
    <br />
    **Event Type:**
    <br />
    `listener.on('commit:created')`
    <br />
    **Supported field types:**<br />`number`, `boolean`, `date`

  </Card>
</CardGroup>

## Parameters

<ParamField path="sheetId" type="string" required>
  The `sheetId` parameter specifies the ID of the sheet you wish to monitor.
</ParamField>

<ParamField path="options.chunkSize" default="3000" type="number" optional>
  The `chunkSize` parameter allows you to specify the quantity of records to in
  each chunk.
</ParamField>

<ParamField path="options.parallel" default="1" type="number" optional>
  The `parallel` parameter allows you to specify the number of chunks to process
  in parallel.
</ParamField>

## API Calls

- `api.sheets.get`

## Imported NPM Packages

- [`@flatfile/api`](https://www.npmjs.com/package/@flatfile/api)
- [`@flatfile/listener`](https://www.npmjs.com/package/@flatfile/listener)
- [`@flatfile/hooks`](https://www.npmjs.com/package/@flatfile/hooks)
- [`@flatfile/plugin-record-hook`](./record-hook)

## Usage

The `autocast` plugin will listen for the `commit:created` event and cast numbers, booleans, and dates to the appropriate Blueprint type.

### Numbers

String numbers (i.e `'1'`), string decimals (i.e `'1.1'`), and string numbers with commas (i.e `'1,000'`) are interpreted as numbers.

### Booleans

`'1'`, `'yes'`, `'true'`, `'on'`, `'t'`, `'y'`, and `1` are interpreted as truthy values.

`'-1'`, `'0'`, `'no'`, `'false'`, `'off'`, `'f'`, `'n'`, `0`, `-1` are interpreted as falsy values.

### Dates

Date strings and numbers are cast to a UTC string. These are example of dates that will be cast:

- `'2023-08-16'` => `'Wed, 16 Aug 2023 00:00:00 GMT'`
- `'08-16-2023'` => `'Wed, 16 Aug 2023 00:00:00 GMT'`
- `'08/16/2023'` => `'Wed, 16 Aug 2023 00:00:00 GMT'`
- `'Aug 16, 2023'` => `'Wed, 16 Aug 2023 00:00:00 GMT'`
- `'August 16, 2023'` => `'Wed, 16 Aug 2023 00:00:00 GMT'`
- `'2023-08-16T00:00:00.000Z'` => `'Wed, 16 Aug 2023 00:00:00 GMT'`
- `1692144000000` => `'Wed, 16 Aug 2023 00:00:00 GMT'`

```SSH install
npm i @flatfile/plugin-autocast
```

```js import
import { autocast } from "@flatfile/plugin-autocast";
```

<CodeGroup>

```js listener.js
listener.use(autocast("us_sp_123456789"));
```

```js listener.js w/ options
listener.use(autocast("us_sp_123456789", { chunkSize: 10000, parallel: 2 }));
```

</CodeGroup>

<CardGroup cols={2}>
  <Card
    title="@flatfile/plugin-autocast"
    href="https://github.com/FlatFilers/flatfile-plugins/tree/main/plugins/autocast"
    icon="github"
  ></Card>
</CardGroup>
