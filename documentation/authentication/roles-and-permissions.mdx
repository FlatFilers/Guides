...
<Info>Workbook grants can only be configured using code. See code example below.</Info>

When creating a guest with workbook access, you can:
- Grant access to specific workbooks within a space
- Grant access to workbooks across multiple spaces
- Reuse the same guest credentials for additional workbook access later

```js
// Example: Creating a guest with access to specific workbooks in multiple spaces
...
// Example: Adding additional workbook access for an existing guest
const addAccess = await api.guests.create({
  environmentId: "us_env_hVXkXs0b",
  email: "guest@example.com",  // Using same email maintains single guest
  name: "Mr. Guest",
  spaces: [
    {
      id: "us_sp_DrdXetPN",
...
<Note>
When granting a guest access to multiple workbooks:
- The same guest credentials can be reused to add access to new workbooks
- A guest can have access to workbooks across different spaces
- Each workbook grant is additive - new grants don't remove existing access
</Note>