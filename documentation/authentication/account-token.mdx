...
PATs follow this format: `pat_xKoBgIYU.OFiv8RpFijALvauOE0QdmEJAr2sjv9K8` where:
- `pat_` is the prefix identifying it as a Personal Access Token
- `xKoBgIYU` is the access token ID
- `.` is the separator
- `OFiv8RpFijALvauOE0QdmEJAr2sjv9K8` is the secret token part
...
- PATs are only shown once at the time of generation and cannot be retrieved afterward
- The token is securely hashed using argon2 before being stored in the database
- PATs have specific exclusions and cannot be used to:
  - Create more PATs
  - Retrieve environment API keys
