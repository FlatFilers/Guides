...
Furthermore, the default page size for Spaces has been set to 10, aligning it with the specifications outlined in the API documentation.
    ...
    >
      The API specification has been updated to facilitate the mapping of enum values of various types such as `string`, `integer`, or `boolea`n. This modification effectively resolves a server error response that was previously encountered when utilizing such `enum` values within the API.
    ...
    >
      In the recent change for instance extraction, we noticed header detection involved parsing the entire file instead of limiting the process to the first N lines. This methodology posed challenges for large files, leading to memory issues.
