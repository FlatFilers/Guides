...
<div style={{ display: "table-row", width: "auto" }}>
    ...
    >
        Introducing support for finding empty fields in ffql using the syntax: `filter: first_name eq ""`. Now, you can easily query and filter records with empty values.
    </div>
...
<div style={{ display: "table-row", width: "auto" }}>
    ...
    >
        Increased mapping concurrency from 2 to 5 due to significant reductions in database load from mapping operations. This adjustment improves processing efficiency.
    </div>
