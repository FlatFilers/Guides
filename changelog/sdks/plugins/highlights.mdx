...
<Update label="05-16-2024" tags={["export"]}>
`@flatfile/plugin-export-workbook@5.1.0`

The export workbook plugin now includes an option to export workbook data without record messages. This feature gives you more control over the exported data format by allowing you to exclude validation messages and other record-level annotations from the export.

To use this feature, set the `includeRecordMessages` option to `false` when configuring the plugin:

```javascript
exportWorkbookPlugin({
  includeRecordMessages: false,
  // other options...
})
```

This is particularly useful when you need cleaner exports or when sending data to systems that don't need the validation message information.
</Update>
