...
<div style={{ display: "table-row", width: "auto" }}>
    ...
    <div style={{ float: "left", display: "table-column", paddingLeft: "30px", width: "calc(80% - 30px)" }}>
     ...
     The common utils now include a new `deleteRecords` utility and a `chunkify` utility. This release also fixed a typing bug in `processRecords` when updating the records by removing the `updatedAt` field.
...
<div style={{ display: "table-row", width: "auto" }}>
    <Snippet file="chips/transform.mdx" />
    <div style={{ float: "left", display: "table-column", paddingLeft: "30px", width: "calc(80% - 30px)" }}>
      `@flatfile/plugin-autocast@0.7.8`

      - Automatic Data Type Conversion: Leveraging Blueprints, this release introduces automatic conversion of data types in Sheets. This ensures that data is consistently formatted and managed according to predefined standards, enhancing data accuracy and usability.

      - Advanced Monitoring and Processing Features:
        - Field Monitoring: We now support monitoring specific fields within Sheets, allowing for targeted data analysis and real-time error detection.
        - Chunk Processing: To improve handling of large datasets, chunk processing has been implemented, optimizing memory usage and efficiency.
        - Parallel Processing Capabilities: Enhance processing speed and efficiency with new parallel processing capabilities, which allow simultaneous data operations, reducing overall processing time.

      These updates significantly improve data handling flexibility and system performance, ensuring a smoother and more efficient user experience.
...
<div style={{ display: "table-row", width: "auto" }}>
    ...
    <div style={{ float: "left", display: "table-column", paddingLeft: "30px", width: "calc(80% - 30px)" }}>
      ...
      This PR improves the error handling in the @flatfile/plugin-foreign-db-extractor plugin, correctly set the file status on job completion, and polls for a DB user since it may take time for the restored DB to become available.
