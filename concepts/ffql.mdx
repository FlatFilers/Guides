...
FFQL (Flatfile Query Language) is Flatfile's custom query language used for filtering data in <Tooltip tip="A collection of fields...">[Sheets](../blueprint/overview#terms)</Tooltip>.
It's logical, composable, and human-readable.
Here's an example using the `GET sheets/<sheetId>/records` endpoint:

```http request
GET sheets/<sheetId>/records?q=first_name eq Bender and last_name eq Rodriguez
```

The query portion is below:
```sql
first_name eq Bender and last_name eq Rodriguez
```
This query searches for rows where the First Name column is "Bender" and the Last Name column is "Rodriguez".
...
## The `is` Operator: Querying for Message Status
