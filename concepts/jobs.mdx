...
A job can be initiated by a user triggering an Action in the UI, by a listener reacting to an Event, or by the Flatfile Platform itself.
...
The Flatfile UI provides a number of Actions that trigger Jobs :

1. Mapping data from one workbook to another (moving data from a workbook to a Workbook)
2. Deleting data from a Workbook (operating on data in a single Workbook)
3. Exporting data from a Workbook
4. Find and Replacing data in a Sheet
5. Apply a Mutate function to data in a Sheet

### Developer-created Actions

Additional Actions can be mounted on Sheets, Workbooks, Spaces, and files. Learn more about [Actions](../concepts/actions).

### System-created

The Flatfile Platform itself creates Jobs to perform extractions on supported file types when a file is created.
Additionally, the Platform initiates a Job with operation `space:created` when a Space is created, without executing the Job. A listener is expected to be set up to do so.

### Listener-created

Listeners can also be set up by developers to react to Events and create new Jobs.
