...
Jobs can be managed via SDK/API. For a complete list of ways to interact with Jobs please see our [API Reference](https://reference.flatfile.com/docs/api) . Commonly, Jobs are acknowledged, progressed, and then completed, or failed. Here's look at those steps.
