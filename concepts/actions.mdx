...
<ParamField path="constraints" type="array[{}]">
  `[{ type: 'hasAllValid' }]`: Adding this constraint will disable a Sheet Action when there are invalid
  records.
  
  `[{ type: 'hasSelection' }]`: Adding this constraint will disable a Sheet Action when no records in the Sheet
  are selected.
</ParamField>

<ParamField path="mode" type="string" default="background">
  Can be `foreground`, `background` or `toolbar-blocking`. Foreground mode will
  prevent interacting with the entire resource until complete.
</ParamField>

<ParamField path="requireAllValid" type="boolean" default="false">
  Determines if a sheet or a workbook level action should be disabled until all rows are valid.
  If set to `true` on a sheet-level action, that action will be disabled untill all rows are valid on that sheet only.
  If set to `true` on a workbook-level action. that action will be disabled untill all rows on all sheets of that workbook are valid.
</ParamField>
