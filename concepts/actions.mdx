...
An Action is a code-based operation that runs where that Action is mounted. Actions run when a user clicks the corresponding user prompt in Flatfile. Additional Actions can be mounted on a Sheet, a Workbook, or a file.

## Action Types

### Built-in Actions

Sheets and files come with two default built-in actions:

1. Delete
2. Download

When these built-in actions are clicked, they trigger events. You can listen for these events and take appropriate actions if required.
