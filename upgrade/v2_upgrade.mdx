...
<Note>
    If you're using V2 Record Hooks `mode` to split hooks on "init" or on "change", Platform does not have this mode functionality. It does, however, have better overall hooks performance and a new [Bulk Record Hook](/plugins/transform/record-hook#additional-options) method that make `mode` unnecessary.
</Note>
