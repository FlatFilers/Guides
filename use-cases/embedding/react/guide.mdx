...
<Warning>While a client-side listener will be able to handle a lot of the interaction and updates in the browser, there may be operations it can't perform due to these limitations. For tasks that need to access server resources or capabilities, a server-side listener will also be required. [Learn more](../../../developer-tools/client_server_side)</Warning>
...
The `useSpace` hook will create a Space component and pass it the configuration and listener as options:
...
| [workbook](#part-1-build-a-workbook)       | Shape of data you will be receiving                                       | Flatfile.WorkbookConfig | Yes      |
| [listener](#build-the-listener)        | Event listener for responding to Events                                | Listener                  | No       |
| [closeSpace](#closespace)    | Options for when to close the iframe and a callback function when session is complete | operation: string, onClose: (data: any) => void | No       |
| [error](#error)          | Error element to display if component throws an error                              | (error: Error \| string) => React.ReactElement | No       |
| [spaceInfo](#spaceinfo)      | Additional metadata to be passed to the space                              |  ISpaceInfo | No       |
| [iframeStyles](#iframestyles)   | Styles the iframe using CSS Properties                                     | React.CSSProperties     | No       |
| [themeConfig](#themeconfig)    | Theme determining the look and feel of the Space                            | IThemeConfig            | No       |
| [sidebarConfig](#sidebarconfig)  | Options for show or hide various UI elements                               | ISidebarConfig          | No       |
| [document](#document)       | Document to pass to space                                                 | Flatfile.DocumentConfig | No       |
| [loading](#loading)       | Loading element to display while space loads                              | React.ReactElement      | No       |
...
<Note>If multiple values are provided for defaultPage, it with prioritize in the following order: Sheet, Workbook, Document, Checklist.</Note>
