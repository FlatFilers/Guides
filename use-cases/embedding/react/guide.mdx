...
The `useSpace` hook configures a Space component. You many opt either to load an existing space, or create a new one. 

Here's an example that creates a new space:
...
To reuse a space we would remove the `name`, `publishableKey`, and `workbook` parameters and add a `space` configuration.

Please note the space configuration uses your `secretKey` instead of your `publishableKey`.
...
space: {
id: 'us_sp_DrdXetPN',
accessToken: 'sk_23ghsyuyshs7dcrty'
}
})
```

<Warning>
  Unlike the `publishableKey`, the `secretKey` shouldn't be exposed in your browser. You must use an environment variable to protect your secret key.
</Warning>
